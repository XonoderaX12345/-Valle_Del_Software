...
app.get("/kpis", requireAuth, requireRoles(["coordinador"]), async (_req, res) => {
  const rows = await prisma.kpiSnapshot.findMany({ orderBy: { createdAt: "desc" } });
  res.json(rows);
});

app.post("/kpis", requireAuth, requireRoles(["coordinador"]), async (req, res) => {
  const parsed = itemSchema.safeParse(req.body);
  if (!parsed.success) return res.status(400).json({ error: "Invalid payload" });

  const row = await prisma.kpiSnapshot.create({
