...
app.get("/health", async (_req, res) => {
  ...
  res.json({ service: "cohort-service", status: "ok" });
});

app.get("/cohortes", requireAuth, requireRoles(["coordinador","mentor","estudiante"]), async (_req, res) => {
  const rows = await prisma.cohort.findMany({ orderBy: { createdAt: "desc" } });
  res.json(rows);
});

app.post("/cohortes", requireAuth, requireRoles(["coordinador"]), async (req, res) => {
...
app.listen(port, () => {
  console.log(`cohort-service listening on ${port}`);
});
