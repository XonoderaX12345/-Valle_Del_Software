...
app.get("/health", async (_req, res) => {
  ...
  res.json({ service: "activity-service", status: "ok" });
});

app.get("/activities", requireAuth, requireRoles(["coordinador","mentor","estudiante"]), async (_req, res) => {
  const rows = await prisma.activity.findMany({ orderBy: { createdAt: "desc" } });
  res.json(rows);
});

app.post("/activities", requireAuth, requireRoles(["coordinador","mentor"]), async (req, res) => {
...
app.listen(port, () => {
  console.log(`activity-service listening on ${port}`);
});
