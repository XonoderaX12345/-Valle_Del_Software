...
app.get("/health", async (_req, res) => {
  ...
  res.json({ service: "certificate-service", status: "ok" });
});

app.get("/certificates", requireAuth, requireRoles(["coordinador","estudiante","mentor"]), async (_req, res) => {
  const rows = await prisma.certificate.findMany({ orderBy: { createdAt: "desc" } });
  res.json(rows);
});

app.post("/certificates", requireAuth, requireRoles(["coordinador"]), async (req, res) => {
...
app.listen(port, () => {
  console.log(`certificate-service listening on ${port}`);
});
