...
app.get("/health", async (_req, res) => {
  ...
  res.json({ service: "pmo-service", status: "ok" });
});

app.get("/projects", requireAuth, requireRoles(["coordinador","cliente","mentor"]), async (_req, res) => {
  const rows = await prisma.project.findMany({ orderBy: { createdAt: "desc" } });
  res.json(rows);
});

app.post("/projects", requireAuth, requireRoles(["coordinador","cliente"]), async (req, res) => {
...
app.listen(port, () => {
  console.log(`pmo-service listening on ${port}`);
});
