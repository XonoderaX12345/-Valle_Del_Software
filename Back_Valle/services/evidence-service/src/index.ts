...
app.get("/health", async (_req, res) => {
  ...
  res.json({ service: "evidence-service", status: "ok" });
});

app.get("/evidences", requireAuth, requireRoles(["coordinador","mentor","estudiante"]), async (_req, res) => {
  const rows = await prisma.evidence.findMany({ orderBy: { createdAt: "desc" } });
  res.json(rows);
});

app.post("/evidences", requireAuth, requireRoles(["estudiante"]), async (req, res) => {
